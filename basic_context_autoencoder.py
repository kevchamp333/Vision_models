# -*- coding: utf-8 -*-
"""
Created on Fri Jun 10 17:33:17 2022
...
# Hyperparameter
context_autoencoder = Autoencoder().to(device)
optimizer = torch.optim.Adam(context_autoencoder.parameters(), lr = 0.005)
criterion = nn.MSELoss()

def mask(img):
    transform = T.RandomErasing(p=1, scale = (0.01, 0.8))
    noisy_img = transform(img.reshape(img.shape[0], 3, 32, 32))
...
for epoch in range(1, EPOCH+1):
    loss = train(context_autoencoder, train_dataloader)
...
noisy_x = mask(original_x.unsqueeze(0)).to(device)
_, recovered_x = context_autoencoder(noisy_x.reshape(1, 3, 32, 32))
...
torch.save(context_autoencoder.state_dict(), save_module_PATH + 'cifar10_context_ae.pt')

# # Load Model
load_module_PATH = r'C:\Users\WooYoungHwang\Desktop\SPS\2022 spring semester\다변량통계분석및데이터마이닝\과제\과제7\코드'  
context_autoencoder = Autoencoder()
context_autoencoder.load_state_dict(torch.load(load_module_PATH + 'cifar10_context_ae.pt'))
context_autoencoder = context_autoencoder.to(device)
...
for epoch in range(1, epoch+1):
    ...
    for step, (x, label) in enumerate(train_dataloader, 0):
        ...
        y = y.to(device)
        label = label.to(device)
        encoded, _ = context_autoencoder(noisy_x.reshape(x.shape[0], 3, 32, 32).to(device))
...
# # Save Model
save_module_PATH = r'C:\Users\WooYoungHwang\Desktop\SPS\2022 spring semester\다변량통계분석및데이터마이닝\과제\과제7\코드'           #모듈 저장 위치 지정
torch.save(downstream.state_dict(), save_module_PATH + 'cifar10_context_ae_downstream.pt')

# # Load Model
load_module_PATH = r'C:\Users\WooYoungHwang\Desktop\SPS\2022 spring semester\다변량통계분석및데이터마이닝\과제\과제7\코드'  
downstream = DownstreamTask()
downstream.load_state_dict(torch.load(load_module_PATH + 'cifar10_context_ae_downstream.pt'))
...
def mask_(img):
    transform = T.RandomErasing(p=1, scale = (0.01, 0.8))
    noisy_img = transform(img.reshape(1, 3, 32, 32))
...
for step, (x, label) in enumerate(test_dataset, 0):
    noisy_x = mask_(torch.tensor(x))  # 입력에 노이즈 더하기
    noisy_x = (noisy_x/255).unsqueeze(0)
    
    encoded, decoded = context_autoencoder(noisy_x.reshape(1, 3, 32, 32).to(device))
    output = downstream(encoded)

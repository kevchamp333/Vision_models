# -*- coding: utf-8 -*-
"""
Created on Fri Jun 10 00:59:49 2022
...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun  9 22:01:42 2022
...
class Autoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # # decoded = self.unflatten(encoded)
        # decoded = decoded.reshape(decoded.shape[0], 6, 4, 4)
        decoded = self.decoder(encoded)
        
        return encoded, decoded

# Hyperparameter
denoising_autoencoder = Autoencoder().to(device)
optimizer = torch.optim.Adam(denoising_autoencoder.parameters(), lr = 0.005)
...
# =============================================================================
# Pretrain denoising autoencoder 
# =============================================================================
def train(autoencoder, train_loader):
    ...
    for step, (x, label) in enumerate(train_loader, 0):
        ...
        encoded, decoded = autoencoder(noisy_x.reshape(x.shape[0], 3, 32, 32).to(device))

        loss = criterion(decoded, y.reshape(x.shape[0], 3, 32, 32))
...
for epoch in range(1, EPOCH+1):
    loss = train(denoising_autoencoder, train_dataloader)
    print("[Epoch {}] loss:{}".format(epoch, loss))


# =============================================================================
# Visualize Denoising Autoencoder result
# =============================================================================
...
_, recovered_x = denoising_autoencoder(noisy_x.unsqueeze(0).reshape(1, 3, 32, 32))
...
noisy_img = np.reshape(noisy_x.to("cpu").data.numpy(), (32, 32, 3))
recovered_img = np.reshape(recovered_x.to("cpu").data.numpy(), (32, 32, 3))
...
torch.save(denoising_autoencoder.state_dict(), save_module_PATH + 'cifar10_denoising_ae.pt')
...
denoising_autoencoder.load_state_dict(torch.load(load_module_PATH + 'cifar10_denoising_ae.pt'))
denoising_autoencoder = denoising_autoencoder.to(device)
...
# =============================================================================
def add_noise_d(img):
    noise = torch.randn(torch.tensor(img).size()) * 0.1
    noisy_img = torch.tensor(img) + noise
    return noisy_img
...
for epoch in range(1, epoch+1):
    ...
    for step, (x, label) in enumerate(train_dataloader, 0):
        ...
        y = y.to(device)
        label = label.to(device)
        encoded, _ = denoising_autoencoder(noisy_x.reshape(x.shape[0], 3, 32, 32).to(device))
...
# # Save Model
save_module_PATH = r'C:\Users\WooYoungHwang\Desktop\SPS\2022 spring semester\다변량통계분석및데이터마이닝\과제\과제7\코드'           #모듈 저장 위치 지정
torch.save(downstream.state_dict(), save_module_PATH + 'cifar10_denoising_ae_downstream.pt')

# # Load Model
load_module_PATH = r'C:\Users\WooYoungHwang\Desktop\SPS\2022 spring semester\다변량통계분석및데이터마이닝\과제\과제7\코드'  
downstream = DownstreamTask()
downstream.load_state_dict(torch.load(load_module_PATH + 'cifar10_denoising_ae_downstream.pt'))
...
for step, (x, label) in enumerate(test_dataset, 0):
    noisy_x = add_noise_d(x)  # 입력에 노이즈 더하기
    noisy_x = (noisy_x/255).unsqueeze(0)
    
    encoded, decoded = denoising_autoencoder(noisy_x.reshape(1, 3, 32, 32).to(device))
    output = downstream(encoded)
